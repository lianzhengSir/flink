 ...
 * @author zlz
 * @Description
 * @since 2021/8/5 15:55
 */
public class Test3 {
    static  String filePreFixPath = Thread.currentThread().getContextClassLoader().getResource("").getPath();
...
//        final SQLExecutor executor = new SQLExecutor(tEnv);
//        String file = filePreFixPath+"/testTopN.sql";
//        Path path = new Path(file);
//        String sql;
//        try (FSDataInputStream inputStream = path.getFileSystem().open(path)) {
//            sql = IOUtils.toString(inputStream, Charset.forName("UTF-8"));
//        }
        //executor.execute(sql);

        tEnv.sqlUpdate("CREATE TABLE csv_source\n" +
            "(\n" +
            "    user_id     bigint,\n" +
            "    item_id     bigint,\n" +
            "    category_id bigint,\n" +
        ...
        tEnv.sqlUpdate("CREATE TABLE `dim_behavior` (\n" +
            "  `id` int  ,\n" +
            "  `c_name` varchar  ,\n" +
            "  `e_name` varchar  \n" +
            ")WITH (\n" +
        ...
        tEnv.sqlUpdate( "CREATE TABLE `result_1` (\n" +
            "  `behavior` varchar  ,\n" +
...
//            "  'update-mode' = 'upsert' -- 指定为插入更新模式 \n"+
        ...
        DataStream<Row> ds = tEnv.toRetractStream(group, Row.class).flatMap(new FlatMapFunction<Tuple2<Boolean, Row>, Row>() {
            @Override
